...
vlc_module_begin ()
    ...
    add_string( SOUT_CFG_PREFIX "dst", "", DEST_TEXT,
                DEST_LONGTEXT, true )
    add_string( SOUT_CFG_PREFIX "src", "", DEST_TEXT,
                DEST_LONGTEXT, true )
    ...
    /*edit starts*/
    add_integer( SOUT_CFG_PREFIX "port-audio-out", 0, PORT_AUDIO_TEXT,
                 PORT_AUDIO_LONGTEXT, true )
    add_integer( SOUT_CFG_PREFIX "port-video-out", 0, PORT_VIDEO_TEXT,
                 PORT_VIDEO_LONGTEXT, true )
    add_integer( SOUT_CFG_PREFIX "sequence-video", 0, PORT_VIDEO_TEXT,
                 PORT_VIDEO_LONGTEXT, true )
    add_integer( SOUT_CFG_PREFIX "sequence-audio", 0, PORT_VIDEO_TEXT,
                 PORT_VIDEO_LONGTEXT, true )
    add_integer( SOUT_CFG_PREFIX "ssrc", 0, PORT_VIDEO_TEXT,
                 PORT_VIDEO_LONGTEXT, true )
    /*edit ends*/
...
/*  "dst", "src", "name", "port", "port-audio", "port-video", "port-audio-out", "port-video-out", "sequence-video", "sequence-audio", "ssrc", "*sdp", "ttl", "mux", */
    "dst", "src", "name", "cat", "port", "port-audio", "port-video", "port-audio-out", "port-video-out", "sequence-video", "sequence-audio", "ssrc", "*sdp", "ttl", "mux",
...
struct sout_stream_sys_t
{
    ...
    uint16_t  o_port_audio;
    uint16_t  o_port_video;
    uint16_t  o_sequence_audio;
    uint16_t  o_sequence_video;
    uint32_t  o_ssrc;
    uint8_t   proto;
    bool      rtcp_mux;
    bool      b_latm;
...
static int Open( vlc_object_t *p_this )
{
    ...
    p_sys->psz_source = var_GetNonEmptyString( p_stream, SOUT_CFG_PREFIX "src" );
    ...
    p_sys->o_port_audio = var_GetInteger( p_stream, SOUT_CFG_PREFIX "port-audio-out" );
    p_sys->o_port_video = var_GetInteger( p_stream, SOUT_CFG_PREFIX "port-video-out" );
    p_sys->o_sequence_video = var_GetInteger( p_stream, SOUT_CFG_PREFIX "sequence-video" );
    p_sys->o_sequence_audio = var_GetInteger( p_stream, SOUT_CFG_PREFIX "sequence-audio" );
    p_sys->o_ssrc       = var_GetInteger( p_stream, SOUT_CFG_PREFIX "ssrc" );
...
static sout_stream_id_t *Add( sout_stream_t *p_stream, es_format_t *p_fmt )
{
    ...
    */
    if (!p_sys->psz_source){
        ...
        id->i_sequence = (p_fmt->i_cat == VIDEO_ES)? p_sys->o_sequence_video : p_sys->o_sequence_audio;
       *(uint32_t*)&id->ssrc = p_sys->o_ssrc;
    }
        ...
        switch( p_sys->proto )
        {
            ...
            default:
            {
                ...
                if (! p_sys->psz_source)
                   ...
                   fd = net_OpenDgram_ttl( p_stream, p_sys->psz_source, o_port,
                           p_sys->psz_destination, i_port, ttl, p_sys->proto );
        //             if (fd != -1 && ttl > 0)
        //             net_SetMcastHopLimit( p_stream, fd, AF_INET, ttl ); //BUG: not ipv6 compatible
                }
                */
                if( fd == -1 )
                {
