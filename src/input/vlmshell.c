...
static int ExecuteShowPos( vlm_t *p_vlm, vlm_message_t **pp_status ){
    int i , j;
    for( j = 0; j < p_vlm->i_media; j++ ) {
       vlm_media_sys_t *p_media =  p_vlm->media[j];

        for( i = 0; i < p_media->i_instance; i++ ) {
           vlm_media_instance_sys_t *p_instance = p_media->instance[i];
           if( p_instance->p_input ) {
               int64_t pos = var_GetTime( p_instance->p_input, "time" );
               p_vlm->ckpt_pos = pos;
               return VLC_SUCCESS;
           }
        }
...
#define IS(txt) ( !strcmp( ppsz_arg[i_index], (txt) ) )
 ...
 // if( !IS("play") && !IS("stop") && !IS("pause") && !IS("seek") )
    if( !IS("play") && !IS("play_at") && !IS("stop") && !IS("pause") && !IS("seek") )
    {
        i_index = 1;
        psz_instance = ppsz_arg[0];

        //if( i_index >= i_arg || ( !IS("play") && !IS("stop") && !IS("pause") && !IS("seek") ) )
        if( i_index >= i_arg || ( !IS("play") && !IS("play_at") && !IS("stop") && !IS("pause") && !IS("seek") ) )
...
#undef IS
    ...
    if( !strcmp( psz_control, "play" ) || !strcmp( psz_control, "play_at" ) )
    {
        int i_input_index = 0;
        int64_t pos = -1;
        int i;
        ...
        if ( ! strcmp( psz_control, "play_at" ) ) {
            if ( psz_argument )
               sscanf(psz_argument, "%lld", &pos);
        } else if( ( psz_argument && sscanf(psz_argument, "%d", &i) == 1 ) && i > 0 && i-1 < p_media->cfg.i_input )
        {
            i_input_index = i-1;
        }
        ...
        if( p_media->cfg.b_vod )
            //i_result = vlm_ControlInternal( p_vlm, VLM_START_MEDIA_VOD_INSTANCE, p_media->cfg.id, psz_instance, i_input_index, NULL );    // we should get here now
              i_result = vlm_ControlInternal( p_vlm, VLM_START_MEDIA_VOD_INSTANCE, p_media->cfg.id, psz_instance, i_input_index, NULL, pos );    // we should get here now
        else
            //i_result = vlm_ControlInternal( p_vlm, VLM_START_MEDIA_BROADCAST_INSTANCE, p_media->cfg.id, psz_instance, i_input_index );
              i_result = vlm_ControlInternal( p_vlm, VLM_START_MEDIA_BROADCAST_INSTANCE, p_media->cfg.id, psz_instance, i_input_index, pos );  
...
#define IF_EXECUTE( name, check, cmd ) if( !strcmp(ppsz_command[0], name ) ) { if( (check) ) goto syntax_error;  if( (cmd) ) goto error; goto success; }
    ...
    else IF_EXECUTE( "show_pos", (i_command > 1),    ExecuteShowPos(p_vlm, &p_message) )
    else
    {
