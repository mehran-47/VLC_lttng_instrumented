...
static void StartTitle( input_thread_t * p_input )
{
    ...
    */
    ...
    int64_t resume_time = var_GetTime( p_input, "resume-time" );
    if (resume_time > 0) {
        msg_Info( p_input, "StartTitle: attempting resume");
        p_input->p->i_start = resume_time;
        var_SetTime( p_input, "resume-time", (int64_t) 0 );
    } else
       p_input->p->i_start = (int64_t)(1000000.0
                                         * var_GetFloat( p_input, "start-time" ));
    msg_Info( p_input, "StartTitle: start time is %lld",  p_input->p->i_start );
